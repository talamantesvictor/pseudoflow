...
export function grapher(sentences: atype.SentencesNode[], backLayer: Konva.Layer, frontLayer: Konva.Layer, size: number, palette: ChartPalette): Vector {
   let flowLayer = backLayer;
   let symbolsLayer = frontLayer;
   let runningSentences = [...sentences];
   let baseSize = size;
   let defaultVerticalSpace = baseSize * 0.2;
   let defaultHorizontalSpace = baseSize * 0.7;
   let chartFontSize = Math.max(12, Math.min(32, Math.round(baseSize * 0.04)));
   let columnSymbol: Rect[] = [];
   let chartPalette = palette;
   ...
   function addSymbol(symbol: any): Rect {
      symbolsLayer.add(symbol);
      
      const returnRect: Rect = {
         x: symbol.attrs.x,
         y: symbol.attrs.y,
   ...
   function addFlow(rect1: Rect, rect2: Rect): void {
   ...
   function setNestedReturnFlows(destPosition: Vector): boolean {
      let flowFound = false;
   ...
   function getColumnFromPosX(x: number): number {
